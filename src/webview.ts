...
export function createDiagramPanel(extensionUri: vscode.Uri): vscode.WebviewPanel | undefined {
  ...
  const updateWebview = async () => {
    try {
      const experiment = await parseExperiment(readmePath);
...
function getWebviewContent(webview: vscode.Webview, extensionUri: vscode.Uri, experiment: Experiment): string {
  const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'out', 'main.js'));
  const nonce = getNonce();
  const tableHtml = generateDiagramHtml(experiment);
 ...
 */
function findReadmePath(currentPath: string): string | null {
